...
public class GoodsDetailController {
	...
	@RequestMapping("/com/detail/selectGoodsInfo/{cafe_idx}")//요청 url의미(value의 주소를 호출하게 되면 해당 메서드 실행
	   public ModelAndView selectGoodsInfo(@PathVariable("cafe_idx") int cafe_idx,
	      ...
	      mv.addObject("review", review);//카페번호의 옵션이름 별 전체리뷰, 별점
			...
			/*
			 * List<Map<String,Object>> gzim =
			 * goodsDetailService.getZzim(commandMap.getMap()); mv.addObject("gzim", gzim);
			 */
	      
			/*
			 * mv.addObject("userInfo", userInfoService.selectUserInfo(commandMap.getMap(),
			 * session));
			 */
	...
	/*
	 * @RequestMapping(value="/setZzim") public ModelAndView setZzim(CommandMap
	 * commandMap, HttpServletRequest request) throws Exception { ModelAndView mv =
	 * new ModelAndView("redirect:/com/detail/selectGoodsInfo/{cafe_idx}");
	 * 
	 * String id = request.getParameter("m_id");
	 * 
	 * commandMap.put("m_id", id);
	 * 
	 * goodsDetailService.setZzim(commandMap.getMap(), request);
	 * 
	 * return mv; }
	 * 
	 * @PostMapping(value = "/delZzim") public ModelAndView delZzim(CommandMap
	 * commandMap, HttpSession session) throws Exception { ModelAndView mv = new
	 * ModelAndView("redirect:/com/detail/selectGoodsInfo/{cafe_idx}");//리다이렉트
	 * 
	 * String m_id = (String)session.getAttribute("m_id"); commandMap.put("m_id",
	 * m_id);
	 * 
	 * goodsDetailService.delZzim(commandMap.getMap()); return mv; }
	 */
	@RequestMapping("/com/detail/insertZzim/{cafe_idx}")
    public ModelAndView insertZzim(@PathVariable("cafe_idx") int cafe_idx, CommandMap commandMap, HttpSession session)throws Exception{//여러 데이터 맵형식 저장
